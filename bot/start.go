...
//
// Назначение (бизнес): пользователь генерирует одноразовый код привязки на
// экране «Настройки» в web (тикет 9.6, POST /channels/telegram/link-code —
// реализован в оркестраторе, orchestrator/internal/api/channels.go, здесь НЕ
// реализуется — этот пакет только принимает уже готовый код) и
