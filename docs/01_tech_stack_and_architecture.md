...
|---|---|---|---|
| `machine.commands` | оркестратор → агент | `integration_id` | постановка/отмена задачи, ответы пользователя, согласования |
| `machine.events` | агент → оркестратор | `task_id` (heartbeat/`hello` — `integration_id`) | прогресс, вопросы агента, «завершил», ошибки, heartbeat |
| `notifications.telegram` | оркестратор → бот | `user_id` | уведомления для доставки в Telegram |

> Адресация команд конкретной машине: один топик `machine.commands` с ключом партиции `integration_id`; мост-оркестратор фильтрует по `integration_id` и пушит в нужный WebSocket, оффлайн-команда дочитывается при реконнекте. Окончательная раскладка партиций/ретеншна/ключей зафиксирована в [ADR 0001](adr/0001-redpanda-topics.md) (тикет 3.1).
