...
// Telegram) сюда НЕ входит — это только общий операционный каркас.
//
// Как устроено (тех): пакет даёт три кирпича и связку из них (Service):
...
//   - Service.Run — поднимает chi-роутер с GET /healthz и блокируется до
//     SIGTERM/SIGINT, после чего гасит HTTP-сервер с таймаутом.
//
